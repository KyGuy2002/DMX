...
const int SD_CS_PIN = 5; // Pin for SD card chip select
const int SD_MOSI_PIN = 3; // Pin for SD card MOSI
const int SD_MISO_PIN = 4; // Pin for SD card MISO
const int SD_CLK_PIN = 2; // Pin for SD card clock
...
void setup(void) {  
  ...
  SPI.setSCK(SD_CLK_PIN);  // CLK
  SPI.setTX(SD_MOSI_PIN);  // MOSI
  SPI.setRX(SD_MISO_PIN);  // MISO

  if (!SD.begin(SD_CS_PIN, SPI)) {  // CS
